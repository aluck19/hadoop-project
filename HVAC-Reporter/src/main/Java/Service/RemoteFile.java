...
public class RemoteFile extends AbstractFileFetcher {
    ...
    public File getFile(String classifier) {
        ...
        if(classifier.equalsIgnoreCase("drill")){
            filePath = new Path("hdfs://localhost:9000/user/abhishek/input/firstResult.csv");
        }else{
            filePath = new Path("hdfs://localhost:9000/user/abhishek/secondResult.csv/part-r-00000");
        }
